import {
  ...
  type ScientificObjectKind,
  type ScientificObjectReference,
...
interface StoredRevision<TPayload = unknown> extends ScientificObjectRevision<TPayload> {
  key: string;
}

export interface StoredScientificReference {
  id: string;
  projectId: string;
...
function requireIndexedDb() {
  if (typeof indexedDB === "undefined") {
    throw new Error("LOCAL_OBJECT_STORE_UNAVAILABLE");
  }
}
  ...
  return await new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(REVISIONS_STORE)) {
        const store = db.createObjectStore(REVISIONS_STORE, { keyPath: "key" });
        store.createIndex("byObject", "objectId", { unique: false });
      }

      if (!db.objectStoreNames.contains(REFERENCES_STORE)) {
        const store = db.createObjectStore(REFERENCES_STORE, { keyPath: "id" });
        store.createIndex("byProject", "projectId", { unique: false });
        store.createIndex("byReferencedObject", "reference.objectId", { unique: false });
      }
    };
...
function revisionKey(objectId: string, revision: number) {
...
export async function createLocalScientificObject<TPayload>(input: {
...
}): Promise<ScientificObject<TPayload>> {
  const db = await openDatabase();
  const id = makeId();
  const now = new Date().toISOString();

  const revision: ScientificObjectRevision<TPayload> = {
    objectId: id,
    revision: 1,
  ...
  const object: ScientificObject<TPayload> = {
    id,
...
export async function appendLocalObjectRevision<TPayload>(
  objectId: string,
  payload: TPayload,
  provenance: ScientificProvenance,
  artifacts?: ScientificArtifact[],
): Promise<ScientificObjectRevision<TPayload>> {
  const db = await openDatabase();
  const transaction = db.transaction([OBJECTS_STORE, REVISIONS_STORE], "readwrite");
  const objectStore = transaction.objectStore(OBJECTS_STORE);
  const current = (await requestResult(objectStore.get(objectId))) as ScientificObject | undefined;

  if (!current) {
    transaction.abort();
    db.close();
    throw new Error("SCIENTIFIC_OBJECT_NOT_FOUND");
  }
  ...
  objectStore.put({ ...current, currentRevision: nextRevision, updatedAt: now });
...
export async function getLocalScientificObject<TPayload = unknown>(id: string): Promise<ScientificObject<TPayload> | undefined> {
  ...
  const stored = (await requestResult(transaction.objectStore(OBJECTS_STORE).get(id))) as ScientificObject<TPayload> | undefined;
  ...
  const storedRevision = (await requestResult(
    transaction.objectStore(REVISIONS_STORE).get(revisionKey(id, stored.currentRevision)),
  )) as StoredRevision<TPayload> | undefined;
  await transactionDone(transaction);
  db.close();

  if (!storedRevision) return stored;
  const { key: _key, ...revision } = storedRevision;
  return { ...stored, revision };
}

export async function listLocalScientificObjects<TPayload = unknown>(projectId: string): Promise<Array<ScientificObject<TPayload>>> {
  const db = await openDatabase();
  const transaction = db.transaction(OBJECTS_STORE, "readonly");
  const index = transaction.objectStore(OBJECTS_STORE).index("byProject");
  const objects = (await requestResult(index.getAll(projectId))) as Array<ScientificObject<TPayload>>;
  await transactionDone(transaction);
  db.close();
  return objects.sort((a, b) => (b.updatedAt || "").localeCompare(a.updatedAt || ""));
}

export async function getLocalObjectRevision<TPayload = unknown>(
  objectId: string,
  revision: number,
): Promise<ScientificObjectRevision<TPayload> | undefined> {
  const db = await openDatabase();
  const transaction = db.transaction(REVISIONS_STORE, "readonly");
  const stored = (await requestResult(
    transaction.objectStore(REVISIONS_STORE).get(revisionKey(objectId, revision)),
  )) as StoredRevision<TPayload> | undefined;
  await transactionDone(transaction);
  db.close();
  if (!stored) return undefined;
  const { key: _key, ...result } = stored;
  return result;
}

export async function createLocalScientificReference(input: {
  projectId: string;
  reference: ScientificObjectReference;
  containerObjectId?: string;
  role?: string;
}): Promise<StoredScientificReference> {
  if (input.reference.projectId !== input.projectId) {
    throw new Error("CROSS_PROJECT_REFERENCE_NOT_ALLOWED_V1");
  }

  const db = await openDatabase();
  const stored: StoredScientificReference = {
    id: makeId(),
    projectId: input.projectId,
  ...
  transaction.objectStore(REFERENCES_STORE).put(stored);
  await transactionDone(transaction);
  db.close();
  return stored;
}
